...
test('Render', async () => {
  ...
  expect(wrapper.find('.message-actions').classes()).not.toContain('visible')
})

test('User message', async () => {
  const wrapper = await mount(userMessage)
  expect(wrapper.find('.message').classes()).toContain('user')
  expect(wrapper.find('.message-role').classes()).toContain('user')
  expect(wrapper.find('.message-role').text()).toBe('chat.role.user')
  expect(wrapper.find('.message-role .avatar').exists()).toBe(true)
  expect(wrapper.find('.message-role .logo').exists()).toBe(false)
  expect(wrapper.find('.message-body').text()).toBe('Hello <img src="https://example.com/image.jpg" alt="description">')
  expect(wrapper.find('.message-body .message-transient').exists()).toBe(false)
  expect(wrapper.find('.message-body .toggle-reasoning').exists()).toBe(false)
  expect(wrapper.find('.message-body .think').exists()).toBe(false)
  expect(wrapper.find('.message-actions .copy').exists()).toBe(true)
  expect(wrapper.find('.message-actions .scratchpad').exists()).toBe(false)
  expect(wrapper.find('.message-actions .read').exists()).toBe(false)
  expect(wrapper.find('.message-actions .retry').exists()).toBe(false)
  expect(wrapper.find('.message-actions .fork').exists()).toBe(true)
  expect(wrapper.find('.message-actions .edit').exists()).toBe(true)
  expect(wrapper.find('.message-actions .quote').exists()).toBe(true)
...
test('Assistant text message', async () => {
  ...
  expect(wrapper.find('.message-role').classes()).toContain('assistant')
  expect(wrapper.find('.message-role').text()).toBe('chat.role.assistant')
  expect(wrapper.find('.message-role .avatar').exists()).toBe(true)
  expect(wrapper.find('.message-role .logo').exists()).toBe(true)
  expect(wrapper.find('.message-body').text()).toBe('Hi\n\n\nOne\n\n\nTwo')
  expect(wrapper.find('.message-body .message-transient').exists()).toBe(false)
  expect(wrapper.find('.message-body .toggle-reasoning').exists()).toBe(false)
  expect(wrapper.find('.message-body .think').exists()).toBe(false)
  expect(wrapper.find('.message-actions .copy').exists()).toBe(true)
  expect(wrapper.find('.message-actions .scratchpad').exists()).toBe(true)
  expect(wrapper.find('.message-actions .read').exists()).toBe(true)
  expect(wrapper.find('.message-actions .retry').exists()).toBe(true)
  expect(wrapper.find('.message-actions .fork').exists()).toBe(true)
  expect(wrapper.find('.message-actions .edit').exists()).toBe(true)
...
test('Assistant legacy image message', async () => {
  ...
  expect(wrapper.find('.message-body .image').exists()).toBe(true)
  expect(wrapper.find('.message-body .download').exists()).toBe(true)
  expect(wrapper.find('.message-body .message-transient').exists()).toBe(false)
  expect(wrapper.find('.message-body img').attributes('src')).toBe('https://example.com/image.jpg')
  expect(wrapper.find('.message-actions .copy').exists()).toBe(true)
  expect(wrapper.find('.message-actions .scratchpad').exists()).toBe(false)
  expect(wrapper.find('.message-actions .read').exists()).toBe(false)
  expect(wrapper.find('.message-actions .retry').exists()).toBe(true)
...
test('Assistant image markdown message', async () => {
  ...
  expect(wrapper.find('.message-body .download').exists()).toBe(true)
  expect(wrapper.find('.message-body .message-transient').exists()).toBe(false)
  expect(wrapper.find('.message-body .media-container img').attributes('src')).toBe('https://example.com/image.jpg')
  expect(wrapper.find('.message-actions .copy').exists()).toBe(true)
  expect(wrapper.find('.message-actions .scratchpad').exists()).toBe(true)
  expect(wrapper.find('.message-actions .read').exists()).toBe(true)
  expect(wrapper.find('.message-actions .retry').exists()).toBe(true)
...
test('Assistant image markdown link at beginning message', async () => {
  ...
  expect(wrapper.find('.message-body').text()).toBe('')
  expect(wrapper.find('.message-body .media-container').exists()).toBe(false)
  expect(wrapper.find('.message-body .download').exists()).toBe(false)
  expect(wrapper.find('.message-body .message-transient').exists()).toBe(false)
  expect(wrapper.find('.message-body a').attributes('href')).toBe('url')
  expect(wrapper.find('.message-body a img').attributes('src')).toBe('https://example.com/image.jpg')
})

test('Assistant image markdown link at middle message', async () => {
  const wrapper = await mount(botMessageImageLink2)
  expect(wrapper.find('.message-body').text()).toBe('look: . neat?')
  expect(wrapper.find('.message-body .media-container').exists()).toBe(false)
  expect(wrapper.find('.message-body .download').exists()).toBe(false)
  expect(wrapper.find('.message-body .message-transient').exists()).toBe(false)
  expect(wrapper.find('.message-body a').attributes('href')).toBe('url')
...
test('Assistant video markdown message', async () => {
  ...
  expect(wrapper.find('.message-body').text()).toBe('')
  expect(wrapper.find('.message-body .video').exists()).toBe(true)
  expect(wrapper.find('.message-body .download').exists()).toBe(true)
  expect(wrapper.find('.message-body .message-transient').exists()).toBe(false)
  expect(wrapper.find('.message-body video').attributes('src')).toBe('file:///data/video.mp4')
})
...
test('Assistant image message formats', async () => {
  ...
  expect(wrapper1.find('.message-body img.image').attributes().src).toBe('https://example.com/image.jpg')

  botMessageImageLegacy.setImage('file://image.png')
  const wrapper2 = await mount(botMessageImageLegacy)
  expect(wrapper2.find('.message-body img.image').attributes().src).toBe('file://image.png')

  botMessageImageLegacy.setImage('imageb64')
  const wrapper3 = await mount(botMessageImageLegacy)
  expect(wrapper3.find('.message-body img.image').attributes().src).toBe('data:image/png;base64,imageb64')

  const wrappe4 = await mount(botMessageImage)
  expect(wrappe4.find('.message-body img.image').attributes().src).toBe('https://example.com/image.jpg')

  const wrapper5 = await mount(botMessageImageCode1)
  expect(wrapper5.find('.message-body').text()).toBe('image: <img src="https://example.com/image.jpg" alt="description">')
  expect(wrapper5.find('.message-body img').exists()).toBe(false)
...
test('Assistant image message with HTML code', async () => {
  ...
  expect(wrapper6.find('.message-body').text()).toBe('image:\n<img src="https://example.com/image.jpg" alt="description">')
  expect(wrapper6.find('.message-body img').exists()).toBe(false)

  const wrapper7 = await mount(botMessageVideoCode1)
  expect(wrapper7.find('.message-body').text()).toBe('video: <video controls src="file:///data/video.mp4" />')
  expect(wrapper7.find('.message-body video').exists()).toBe(false)

  const wrapper8 = await mount(botMessageVideoCode3)
  expect(wrapper8.find('.message-body').text()).toBe('video:\n<video controls src="file:///data/video.mp4" />')
  expect(wrapper8.find('.message-body video').exists()).toBe(false)
})

test('Assistant image message with tool media', async () => {

  const wrapper1 = await mount(botMessageToolMedia1)
  expect(wrapper1.find('.message-body').text()).toBe('Here:\nWelcome!')
  expect(wrapper1.findAll('.message-body img').length).toBe(1)

  const wrapper2 = await mount(botMessageToolMedia2)
  expect(wrapper2.find('.message-body').text()).toBe('Sure!\nWelcome!')
  expect(wrapper2.findAll('.message-body img').length).toBe(1)
...
test('Assistant image message with artifact', async () => {
  ...
  expect(wrapper1.find('.message-body').text()).toBe('Here:\ntestTest\nWelcome!')
  expect(wrapper1.findAll('.message-body .artifact').length).toBe(1)

  const wrapper2 = await mount(botMessageToolArtifact2)
  expect(wrapper2.find('.message-body').text()).toBe('Here:\ntest1Test1\ntest2Test2\nWelcome!')
  expect(wrapper2.findAll('.message-body .artifact').length).toBe(2)

  const wrapper3 = await mount(botMessageToolArtifact3)
  expect(wrapper3.find('.message-body').text()).toBe('Here:\ntest1Test1\ntest2Te')
  expect(wrapper3.findAll('.message-body .artifact').length).toBe(2)

  const wrapper4 = await mount(botMessageToolArtifact4)
  expect(wrapper4.find('.message-body').text()).toBe('Here:\ntest1Test1')
  expect(wrapper4.findAll('.message-body .artifact').length).toBe(1)
...
test('Assistant artifact with HTML preview', async () => {
  ...
  expect(wrapper.findAll('.message-body .artifact').length).toBe(1)

  const artifact = wrapper.find('.message-body .artifact')
...
test('Transient message', async () => {
  ...
  expect(wrapper.find('.message-body').text()).toBe('Hi')
  expect(wrapper.find('.message-body .message-transient').exists()).toBe(true)
  expect(wrapper.find('.message-body .message-transient .loader').exists()).toBe(true)
  expect(wrapper.find('.message-body .message-transient').findComponent({ name: 'MessageItemToolBlock' }).exists()).toBe(false)
  expect(wrapper.find('.message-actions .copy').exists()).toBe(false)
  expect(wrapper.find('.message-actions .read').exists()).toBe(false)
  expect(wrapper.find('.message-actions .edit').exists()).toBe(false)
  expect(wrapper.find('.message-actions .tools').exists()).toBe(false)
})
...
test('Toggle actions', async () => {
  ...
  expect(wrapper.find('.message-actions').classes()).toContain('visible')
...
test('Run user actions', async () => {
  ...
  await wrapper.find('.message-actions .edit').trigger('click')
  await nextTick()
  ...
  await wrapper.find('.message-actions .quote').trigger('click')
  expect(chatCallbacksMock.onSetPrompt).toHaveBeenLastCalledWith(userMessage)

  await wrapper.find('.message-actions .delete').trigger('click')
...
test('Run assistant text actions', async () => {
  ...
  expect(wrapper.find('.message-actions .copy').text()).toBe('common.copied')

  // scratchpad
  await wrapper.find('.message-actions .scratchpad').trigger('click')
  ...
  // usage
  await wrapper.find('.message-actions .usage').trigger('click')
  expect(Dialog.show).toHaveBeenCalledTimes(1)

  // read aloud
  await wrapper.find('.message-actions .read').trigger('click')
  expect(readAloudMock).toHaveBeenCalled()

  // retry
  await wrapper.find('.message-actions .retry').trigger('click')
  ...
  // fork
  await wrapper.find('.message-actions .fork').trigger('click')
  expect(chatCallbacksMock.onForkChat).toHaveBeenLastCalledWith(botMessageText)

  // tools
  expect(wrapper.find('.message-body .message-content').findComponent({ name: 'MessageItemToolBlock' }).exists()).toBe(false)
  await wrapper.find('.message-actions .tools').trigger('click')
  expect(wrapper.find('.message-body .message-content').findComponent({ name: 'MessageItemToolBlock' }).exists()).toBe(true)
  await wrapper.find('.message-actions .tools').trigger('click')
  expect(wrapper.find('.message-body .message-content').findComponent({ name: 'MessageItemToolBlock' }).exists()).toBe(false)

  // await wait(1500)
  // expect(wrapper.find('.message-actions .copy').text()).not.toBe('common.copied')
})
...
test('Run assistant legacy image actions', async () => {
  ...
  await wrapper.find('.message-body .download').trigger('click')
...
test('Run assistant image actions', async () => {
  const wrapper = await mount(botMessageImage)
  await wrapper.find('.message-body .copy').trigger('click')
...
test('Run assistant video actions', async () => {
  const wrapper = await mount(botMessageVideoMd)
  expect(wrapper.find('.message-body .copy').exists()).toBe(false)
  await wrapper.find('.message-body .download').trigger('click')
  expect(window.api.file.download).toHaveBeenLastCalledWith({
...
test('Format code', async () => {
  ...
  const wrapper = await mount(botMessageText)
  expect(wrapper.find('.message-body pre[class=hljs] code[class="hljs variable-font-size"]').exists()).toBe(true)
  expect(wrapper.find('.message-body .copy-code').exists()).toBe(true)
  expect(wrapper.find('.message-body .copy-code').text()).toBe('Copy code')
})

test('Format math equations katex', async () => {
  botMessageText.setText('$$\n\\frac{1}{2}\n$$')
  const wrapper = await mount(botMessageText)
  expect(wrapper.find('.message-body p[class=katex-block]').exists()).toBe(true)
})
...
test('Format reasoning message', async () => {
  ...
  expect(wrapper.find('.message-body .toggle-reasoning').exists()).toBe(true)
  expect(wrapper.find('.message-body .think').exists()).toBe(true)
  await (wrapper.find('.message-body .toggle-reasoning').trigger('click'))
  expect(wrapper.find('.message-body .think').exists()).toBe(false)
...
test('Assistant message with table', async () => {
  ...
  expect(wrapper.find('.message-body').text()).toContain('Here is a table:')
  expect(wrapper.find('.message-body').text()).toContain('That\'s it!')
  expect(wrapper.findAll('.message-body .artifact').length).toBe(1)
